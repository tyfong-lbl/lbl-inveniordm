...
check_services_ready() {
    ...
    for service in $services; do
        ...
        container_status=$(docker-compose -f docker-compose.yml ps -q $service | xargs docker inspect -f '{{.State.Status}}' 2>/dev/null || echo "not_found")
        container_running=$(docker-compose -f docker-compose.yml ps -q $service | xargs docker inspect -f '{{.State.Running}}' 2>/dev/null || echo "false")
        restart_count=$(docker-compose -f docker-compose.yml ps -q $service | xargs docker inspect -f '{{.RestartCount}}' 2>/dev/null || echo "0")
        ...
        if [ "$restart_count" -gt "3" ]; then
            ...
            docker-compose -f docker-compose.yml logs --tail=20 $service
            return 1
        fi
        ...
        if [ "$container_running" != "true" ]; then
            if [ "$container_status" = "restarting" ]; then
                ...
                docker-compose -f docker-compose.yml logs --tail=10 $service
                return 1
            else
        ...
        case $service in
            "db")
                if ! docker-compose -f docker-compose.yml exec -T db pg_isready -U lbnl-data-repository > /dev/null 2>&1; then
            ...
            "search")
                ...
                if ! docker-compose -f docker-compose.yml exec -T search curl -k -u admin:${OPENSEARCH_ADMIN_PASSWORD:-admin} -s https://localhost:9200/_cluster/health > /dev/null 2>&1; then
                    echo "WARNING: OpenSearch not ready yet"
                    # Show OpenSearch logs if it's failing
                    echo "Recent OpenSearch logs:"
                    docker-compose -f docker-compose.yml logs --tail=5 search
                    return 1
                fi
                echo "✓ OpenSearch is ready"
                ;;
            "cache")
                if ! docker-compose -f docker-compose.yml exec -T cache redis-cli ping > /dev/null 2>&1; then
                ...
                ;;
            "mq")
                if ! docker-compose -f docker-compose.yml exec -T mq rabbitmq-diagnostics -q ping > /dev/null 2>&1; then
            ...
            "frontend")
                ...
                if ! docker-compose -f docker-compose.yml exec -T frontend curl -k -s https://localhost:443 > /dev/null 2>&1; then
                    echo "WARNING: Frontend not ready yet"
                    echo "Recent frontend logs:"
                    docker-compose -f docker-compose.yml logs --tail=5 frontend
                    return 1
                fi
...
while ! check_services_ready; do
    if [ $elapsed -ge $timeout ]; then
        ...
        for service in frontend search; do
            ...
            docker-compose -f docker-compose.yml logs --tail=30 $service
            echo ""
        done
        exit 1
    fi
