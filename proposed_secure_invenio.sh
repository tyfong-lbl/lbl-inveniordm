...
start_invenio_rdm() {
    ...
    echo "🔍 Checking InvenioRDM startup logs..."
    sleep 5
    ...
    while [ $elapsed -lt $timeout ]; do
        if kill -0 "$invenio_pid" 2>/dev/null; then
            ...
            if curl -s -f "http://127.0.0.1:$port" >/dev/null 2>&1; then
                echo "✅ InvenioRDM is responding on HTTP at port $port"
                return 0
            fi
        else
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --port)
            if [[ -n $2 && $2 =~ ^[0-9]+$ ]]; then
                ...
                if [ "$INVENIO_PORT" != "5000" ]; then
                    echo "⚠️  WARNING: InvenioRDM port changed from 5000 to $INVENIO_PORT"
        ...
        --http-only)
            USE_NGINX_FRONTEND=false
            echo "⚠️  WARNING: HTTP-only mode - InvenioRDM will run without HTTPS"
            shift
            ;;
        --help)
...
if [ "$START_INVENIO" = true ]; then
    if [ "$USE_NGINX_FRONTEND" = true ]; then
        echo "🚀 Starting in FULL mode with HTTPS (containers + nginx + InvenioRDM)"
    else
        echo "🚀 Starting in DIRECT HTTP mode (containers + InvenioRDM on port $INVENIO_PORT)"
    fi
...
if [ "$START_INVENIO" = true ]; then
    ...
    if start_invenio_rdm "$INVENIO_PORT"; then
        ...
        echo "🌐 Service URLs:"
        if [ "$USE_NGINX_FRONTEND" = true ]; then
            echo "   • InvenioRDM (HTTPS):         https://127.0.0.1:5001"
            echo "   • InvenioRDM (HTTP):          http://127.0.0.1:5000"
            echo "   • InvenioRDM Direct:          http://127.0.0.1:$INVENIO_PORT"
        else
            echo "   • InvenioRDM (Direct HTTP):   http://127.0.0.1:$INVENIO_PORT"
        fi
        ...
        echo ""
        if [ "$USE_NGINX_FRONTEND" = true ]; then
            echo "🔒 HTTPS Mode: Access InvenioRDM via https://127.0.0.1:5001"
            echo "   (The nginx frontend handles SSL termination)"
        else
            echo "🔓 HTTP Mode: Access InvenioRDM via http://127.0.0.1:$INVENIO_PORT"
        fi
    ...
    echo "   • To start InvenioRDM with HTTPS:"
    echo "     $0 --invenio"
    echo "   • To start InvenioRDM with HTTP only:"
